...
__all__ = [
    'PACTQuantFunc', 'AlmostSymmQuantFunc', 'PACTQuantize', 'TQTQuantFunc',
    'TQTQuantize'
]
...
class PACTQuantFunc(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        ctx.save_for_backward(where_input_nonclipped, where_input_lo,
                              where_input_hi, clip_gradient, clip_lo)
        ...
        input_unrounded_int = (input.clamp(clip_lo, clip_hi + 1e-7) -
                               clip_lo) / eps
        if noisy:
            noise = torch.rand(input_unrounded_int.size(),
                               device=input_unrounded_int.device) - 0.5
        ...
        ) if floor else input_unrounded_int.round()
    ...
    def backward(ctx, grad_output):
        ...
        grad_upper = torch.where(where_input_hi, grad_output,
                                 zero).sum(dim=reduce_dims).reshape(
                                     clip_lo.shape)
        ...
        grad_lower = torch.where(where_input_lo, grad_output,
                                 zero).sum(dim=reduce_dims).reshape(
                                     clip_lo.shape)
...
class TQTQuantFunc(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, eps, log_t, clip_lo, clip_hi, beta,
                running_grad_var, running_beta, clip_grad_logt, rounding):
        where_input_nonclipped = (input >= clip_lo) * (input < clip_hi)
        where_input_lo = (input < clip_lo)
        where_input_hi = (input >= clip_hi)
        ...
        ctx.save_for_backward(where_input_nonclipped, where_input_lo,
                              where_input_hi, clip_lo, clip_hi, quant_error,
                              beta, running_grad_var, running_beta,
                              clip_grad_logt)
        return input_quant
    ...
    def backward(ctx, grad_output):
        ...
        # clipped high
        grad_logt = clip_hi * torch.where(where_input_hi, grad_output,
                                          zero).sum(dim=reduce_dims).reshape(
                                              clip_lo.shape)
        # clipped low
        grad_logt += clip_lo * torch.where(where_input_lo, grad_output,
                                           zero).sum(dim=reduce_dims).reshape(
                                               clip_lo.shape)
        # unclipped
        grad_logt += torch.where(
            where_input_nonclipped, quant_error * grad_output,
...
def TQTQuantize(input,
                ...
                clip_grad_logt,
                rounding=True):
    return TQTQuantFunc.apply(input, eps, log_t, clip_lo, clip_hi, beta,
                              running_grad_var, running_beta, clip_grad_logt,
                              rounding)
...
class AlmostSymmQuantFunc(torch.autograd.Function):  # TODO: maybe a simple clip_hi = clip_lo - eps?
    ...
    def forward(ctx, clip_lo, n_levels):
        ...
        if n_levels % 2 == 0:
            scale = torch.tensor(-(n_levels - 2) / n_levels,
                                 device=clip_lo.device)
        else:
