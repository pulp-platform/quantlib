...
_PACT_CLASSES = [
    PACTUnsignedAct, PACTAsymmetricAct, PACTConv1d, PACTConv2d, PACTLinear
]


class PACTOptimizerFactory(
        object
        ...
        if rep in self._created_classes.keys():
            ...
            class PACTOptimizer(base_opt_type):
                ...
                def __init__(self, net, pact_decay, *opt_args, **opt_kwargs):
                    ...
                    pact_filter = VariadicOrFilter(
                        *[TypeFilter(t) for t in _PACT_CLASSES])
                    learnable_clipping_params = [
                        b for n in pact_filter(net_nodes)
                        for k, b in n.module.clipping_params.items()
