...
class ModuleFactory(object):
    ...
    def get_module(
        self, class_name: str, training: bool, device: torch.device,
        input_size: Union[Tuple[int, int], Tuple[int, int, int, int]], **kwargs
    ) -> [nn.Module, Union[Tuple[int, int], Tuple[int, int, int, int]]]:

        raise NotImplementedError

    def get_network(
        self, linear_or_conv2d: str, training: bool, device: torch.device,
        input_size: Union[Tuple[int, int], Tuple[int, int, int, int]], **kwargs
    ) -> Tuple[nn.Module, Union[Tuple[int, int], Tuple[int, int, int, int]]]:

        if linear_or_conv2d == 'linear':

            linear1, linear1_size = self.get_module('linear', training, device,
                                                    input_size, **kwargs)
            batchnorm1 = nn.BatchNorm1d(linear1_size[-1])
            act1, act1_size = self.get_module('activation', training, device,
                                              linear1_size, **kwargs)
            linear2, linear2_size = self.get_module('linear', training, device,
                                                    act1_size, **kwargs)
            ...
            conv1, conv1_size = self.get_module('conv2d', training, device,
                                                input_size, **kwargs)
            batchnorm1 = nn.BatchNorm2d(conv1_size[1])
            act1, act1_size = self.get_module('activation', training, device,
                                              conv1_size, **kwargs)
            conv2, conv2_size = self.get_module('conv2d', training, device,
                                                act1_size, **kwargs)
...
class FloatingPointModuleFactory(ModuleFactory):
    ...
    def get_module(self, class_name: str, training: bool, device: torch.device,
                   input_size: Tuple, **kwargs) -> Tuple[nn.Module, Tuple]:
...
class ANAModuleFactory(ModuleFactory):
    ...
    def range(self) -> Range:
        ...
        quant_levels = torch.arange(
            0, 2**self._quantizer_spec['nbits']).to(dtype=torch.float32)
