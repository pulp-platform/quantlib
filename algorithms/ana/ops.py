...
class ANAModule(nn.Module):
    ...
    def __init__(self, quantizer_spec: Dict, noise_type: str, strategy: str):
        ...
        ANAModule.setup_noise(
            self, NoiseType[noise_type.upper()].value,
            ForwardComputationStrategy[strategy.upper()].value
    ...
    def setup_quantizer(anamod: nn.Module, quantizer_spec: Dict) -> None:
        ...
        * quantum (i.e., the precision of the fixed-point representation).
        ...
        # quantization levels
        quant_levels = torch.arange(
            0, 2**quantizer_spec['nbits']).to(dtype=torch.float32)
        ...
        anamod.register_parameter(
            'quant_levels', nn.Parameter(quant_levels, requires_grad=False))

        # thresholds
        thresholds = quant_levels[:-1] + .5
        anamod.register_parameter('thresholds',
                                  nn.Parameter(thresholds, requires_grad=False))
    ...
    def setup_noise(anamod: nn.Module, noise_class, strategy: int):
        ...
        anamod.register_parameter(
            'mi', nn.Parameter(torch.zeros(1), requires_grad=False))
        anamod.register_parameter(
            'sigma', nn.Parameter(torch.ones(1), requires_grad=False))

        anamod.register_parameter(
            'strategy',
            nn.Parameter(torch.Tensor([strategy]).to(torch.int32),
                         requires_grad=False))
...
class ANAActivation(ANAModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_out = self.ana_op(x, self.quant_levels, self.thresholds, self.mi,
                            self.sigma, self.strategy, self.training)
...
class ANALinear(ANAModule):
    ...
    def reset_parameters(self) -> None:
        ...
        self.weight.data = torch.add(
            self.weight.data,
            torch.zeros_like(self.weight.data).uniform_(-stdv, stdv))
...
class _ANAConvNd(ANAModule):
    ...
    def __init__(self, quantizer_spec: Dict, noise_type: str, strategy: str,
                 ...
                 stride: int, padding: int, dilation: int, transposed: bool,
                 output_padding, groups: int, bias: bool):
...
class ANAConv1d(_ANAConvNd):
    ...
    def __init__(self,
        ...
        super(ANAConv1d, self).__init__(quantizer_spec, noise_type, strategy,
                                        in_channels, out_channels, kernel_size,
                                        stride, padding, dilation, False,
                                        _single(0), groups, bias)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return F.conv1d(input, self.weight_maybe_quant * self.eps, self.bias,
...
class ANAConv2d(_ANAConvNd):
    ...
    def __init__(self,
        ...
        super(ANAConv2d, self).__init__(quantizer_spec, noise_type, strategy,
                                        in_channels, out_channels, kernel_size,
                                        stride, padding, dilation, False,
                                        _pair(0), groups, bias)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return F.conv2d(input, self.weight_maybe_quant * self.eps, self.bias,
                        self.stride, self.padding, self.dilation, self.groups)
...
class ANAConv3d(_ANAConvNd):
    ...
    def __init__(self,
        ...
        super(ANAConv3d, self).__init__(quantizer_spec, noise_type, strategy,
                                        in_channels, out_channels, kernel_size,
                                        stride, padding, dilation, False,
