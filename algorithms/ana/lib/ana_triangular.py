...
def forward(x_in, q, t, mi, sigma, strategy, training):
    ...
    if training and sigma != 0.0:
        ...
        cdf_temp = (
            torch.sign(shifted_x_minus_t) * abs_shifted_x_minus_t_over_s *
            (2 - abs_shifted_x_minus_t_over_s) + 1.0) / 2
        cdf = torch.where(abs_shifted_x_minus_t_over_s < 1.0, cdf_temp,
                          (shifted_x_minus_t >= 0.0).float())
    else:
...
def backward(grad_in, x_in, q, t, mi, sigma):
    ...
    if sigma != 0.0:
        ...
        pdf = torch.max(
            torch.Tensor([0.0]).to(device=grad_in.device),
            1.0 - shifted_x_minus_t_over_s) / sigma
    else:
