...
def check_node(n: fx.Node, gm: fx.GraphModule) -> bool:
    ...
    if opcode in FXOPCODES_IO:
        state = False

    elif opcode in FXOPCODE_CALL_MODULE:
...
def find_backward_frontier(n: fx.Node, gm: fx.GraphModule) -> Set[fx.Node]:
    ...
    B = set(filter(lambda p: (p.op in FXOPCODE_CALL_MODULE) and isinstance(gm.get_submodule(target=p.target), EpsTunnel), n.all_input_nodes))
...
def find_forward_frontier(n: fx.Node, gm: fx.GraphModule) -> Set[fx.Node]:
    ...
    F = set(filter(lambda s: (s.op in FXOPCODE_CALL_MODULE) and isinstance(gm.get_submodule(target=s.target), EpsTunnel), n.users))
...
def find_candidate_constructs(gm: fx.GraphModule) -> List[CandidateEpsTunnelConstruct]:
    ...
    V_eps = list(filter(lambda n: (n.op in FXOPCODE_CALL_MODULE) and (isinstance(gm.get_submodule(target=n.target), EpsTunnel)), list(reversed(gm.graph.nodes))))
...
class EpsTunnelConstructFinder(object):

    def __init__(self):
        super(EpsTunnelConstructFinder, self).__init__()

    @staticmethod
    def find(gm: fx.GraphModule) -> List[CandidateEpsTunnelConstruct]:
        candidate_constructs = find_candidate_constructs(gm)
        constructs = list(filter(lambda cc: verify_candidate_construct(cc, gm), candidate_constructs))
        return constructs
