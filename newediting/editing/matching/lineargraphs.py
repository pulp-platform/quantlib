...
from quantlib.newediting.graphs import FXOPCODE_PLACEHOLDER, FXOPCODE_OUTPUT, FXOPCODES_IO, FXOPCODE_CALL_MODULE
...
class LinearGraphMatcher(object):
    ...
    def __init__(self,
        ...
        **Singleton graphs** are a special sub-case of linear graphs. Indeed,
        ...
        self._pattern_gm: fx.GraphModule = self._symbolic_trace_fn(root=pattern_module)
        self._pattern_g: fx.Graph = self._pattern_gm.graph
        if not LinearGraphMatcher._is_linear_graph(self._pattern_g):
            raise ValueError(quantlib_err_header(obj_name=self.__class__.__name__) + "requires a path graph as template.")
        self._pattern_g_anchor = next(iter(reversed(self._pattern_g.nodes)))

        # decompose the nodes of the template part (L-term) of the graph rewriting rule
        self._Kterm_entry = tuple(pn for pn in self._pattern_g.nodes if pn.op in FXOPCODE_PLACEHOLDER)
        self._Kterm_exit  = tuple(pn for pn in self._pattern_g.nodes if pn.op in FXOPCODE_OUTPUT)
        self._Lterm_body  = tuple(pn for pn in self._pattern_g.nodes if (pn not in self._Kterm_entry) and (pn not in self._Kterm_exit))
    ...
    def _check_node_attributes(pn: fx.Node, dn: fx.Node, pattern_gm: fx.GraphModule, data_gm: fx.GraphModule) -> bool:
        ...
        * performing signature matching over the attributes of :math:`H` and
        ...
        if pn.op in FXOPCODES_IO:
            ...
            cond_op = pn.op == dn.op
            try:
                pn_module_object = dict(pattern_gm.named_modules())[pn.target]
                dn_module_object = dict(data_gm.named_modules())[dn.target]
                cond_type = isinstance(dn_module_object, type(pn_module_object))  # Is the data `nn.Module` a sub-class of the query `nn.Module`?
            except KeyError:
                cond_type = False
    ...
    def find(self, data_gm: fx.GraphModule) -> List[Match]:
        ...
        """
        # TODO: we assume that the data graph IR (i.e., granularity) is the same as the pattern graph's
        ...
        def overlaps_with_previous_matches(match: Match) -> bool:
            body_nodes = set(dn for pn, dn in match.nodes_map.items() if (dn.op not in FXOPCODES_IO))
            return any(n in matched_nodes for n in body_nodes)

        matches: List[Match] = []
        for data_g_anchor in reversed(data_gm.graph.nodes):

            candidate_matches = LinearGraphMatcher._match_nodes(self._pattern_g_anchor, data_g_anchor, self._pattern_gm, data_gm)
            for candidate_match in candidate_matches:
                match = Match(anchor=data_g_anchor, nodes_map=candidate_match)
                if not overlaps_with_previous_matches(match):
                    matches.append(match)
                    matched_nodes.union(set(dn for pn, dn in match.nodes_map.items() if pn not in FXOPCODES_IO))
                else:
